...
#define USE_INTRANET // comment to connect through a acess point (images will not work)
#define USE_MOBILEPHONE // comment to use a ssid router instead of a phone

#ifndef USE_MOBILEPHONE
  const char* ssid = "Vodafone-7173DA";
  const char* password = "Vbs4SWybtH";
#endif

#ifdef USE_MOBILEPHONE
  // connect to mobilephone hotspot
  const char* ssid = "MobileRouter-0EA4";
  const char* password = "11657149";
#endif

// use as an access point
const char* AP_ssid = "ESP8266myAP";
...
void loop() {
  if ((millis() - SensorUpdate) >= 50) {
    ...
    */
    ...
    // modo reservatório
    BitsA0 = map(analogRead(PIN_LIGHT_RES), 0, 1023, 100, 0);
